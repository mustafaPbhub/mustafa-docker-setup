$(document).ready(function() {
    ...
    let sliderId = url.split('/#product-').pop();
    if(sliderId != '' && sliderId != null && sliderId != undefined) {
        openSlider(sliderId)
    }
...
$('.video').on('click', function() {
    ...
    let baseUrl = window.location.href.split("#")[0] + "#product-" + productID;
...
function openSlider(sliderId){
    ...
    if(sliderId.length > 0 ){
        SliderVideos  = $(document).find('.video');
        var selectedID = "" ;
        $(SliderVideos).each(function(i,v){
            let id  = $(this).attr('data-product-id');
            if(id == sliderId){
                selectedID =  $(this).attr('data-id');
                initSlider(selectedID);
            }
        })
    }
}
